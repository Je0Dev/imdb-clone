...
public abstract class BaseController implements Initializable {
    ...
    protected RefactoredDataManager dataManager;
    ...
    /**
     * Helper method to add a form field to a grid with consistent styling.
     *
     ...
     */
    protected void addFormField(GridPane grid, String labelText, Node field, int row) {
        Label label = new Label(labelText);
        label.setStyle("-fx-text-fill: white; -fx-font-weight: bold;");
        grid.add(label, 0, row);

        if (field instanceof TextInputControl) {
            field.setStyle("-fx-background-color: #2a2a2a; -fx-text-fill: white; -fx-padding: 5;");
        }
